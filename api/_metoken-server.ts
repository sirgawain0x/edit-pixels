 ...
 */
export async function getMetokenBalance(
  address: string
): Promise<MetokenBalanceResult> {
...
export async function verifyMetokenDebit(
  ...
  try {
    ...
    for (const log of receipt.logs) {
      if (log.address.toLowerCase() !== CRTVAI_DIAMOND_ADDRESS.toLowerCase())
        continue;
