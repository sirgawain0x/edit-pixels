...
export async function GET(request: Request): Promise<Response> {
  ...
  try {
    ...
    const paymentMethod =
      url.searchParams.get('paymentMethod')?.trim() || DEFAULT_PAYMENT_METHOD;
    const partnerUserRef =
      url.searchParams.get('partnerUserRef')?.trim() ||
    ...
    if (!amountValue) {
      return Response.json(
        { error: 'Valid fiat amount is required' },
        { status: 400 }
      );
    }
    ...
    if (!data.paymentLink || typeof data.paymentLink !== 'string') {
      return Response.json(
        { error: 'Invalid onramp order response' },
        { status: 502 }
      );
    }
    ...
    return Response.json(
      { error: 'Failed to generate onramp order' },
      { status: 500 }
    );
  }
}
