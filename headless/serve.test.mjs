...
test('resolveHost accepts PIXELS_HOST', () => {
  assert.equal(resolveHost({}, { PIXELS_HOST: '0.0.0.0' }), '0.0.0.0')
})
...
test('--host takes precedence over PIXELS_HOST', () => {
  assert.equal(resolveHost({ host: '127.0.0.2' }, { PIXELS_HOST: '0.0.0.0' }), '127.0.0.2')
})
...
test('resolveHost rejects empty and missing option values', () => {
  ...
  assert.throws(() => resolveHost({}, { PIXELS_HOST: '' }), /non-empty string/)
})
