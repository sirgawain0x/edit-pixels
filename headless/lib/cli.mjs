 ...
 * args can be appended via PIXELS_CHROME_ARGS (space-separated) — e.g. in
...
export function chromeLaunchArgs() {
  ...
  const replace = process.env.PIXELS_CHROME_ARGS_REPLACE
  ...
  const extra = (process.env.PIXELS_CHROME_ARGS ?? '').split(/\s+/).filter(Boolean)
  return [...base, ...extra]
}
