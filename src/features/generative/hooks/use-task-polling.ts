...
export function useTaskPolling(setTask: SetTask) {
  ...
  const startPolling = useCallback(
    async (taskId: string, getStatus: (signal?: AbortSignal) => Promise<EvolinkTaskDetail>) => {
      ...
      try {
        const result = await pollTask((signal) => getStatus(signal), {
        ...
        // Failed
        const errorMsg = result.error?.message ?? 'Generation failed. Please try again.'
