...
3. On full batch success, if **Lay on timeline after all shots succeed** is checked (default **on**), clips are placed on a new video track aligned to audio.
4. If auto-lay is off, or audio was missing at success time, use **Lay on timeline** after audio is present.
5. Clips remain fully editable (trim, move) after placement.
...
5. Toggle crossfade off/on and re-lay manually to compare (optional).
6. Confirm failed batch does **not** mutate the timeline.
